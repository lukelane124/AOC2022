...
int contained(int doTest)
{
    ...
    {
        /*
        * If any member of the other group is equal to either ends, or found between them it overlaps.
        */
       if (!( (ranges[0][1] < ranges[1][0]) || (ranges[0][0] > ranges[1][1])))
       {
           ret = 1;
       }
       else if (!( (ranges[1][0] < ranges[0][1]) || (ranges[1][1] > ranges[0][0])))
       {
           ret = 1;
       }
    }
